<?php
	...
	if($mode == 'order') {
		...
		$sql = "SELECT fk_soc, total_ht as total, MONTH(date_livraison) as 'month' FROM ".MAIN_DB_PREFIX."commande WHERE fk_statut>0 AND YEAR(date_livraison)=".$year." GROUP BY fk_soc, month ORDER BY date_livraison";
	}
	else{
		//facture payée sur date de facturation
		$sql = "SELECT fk_soc, total, MONTH(datef) as 'month' FROM ".MAIN_DB_PREFIX."facture WHERE fk_statut>0 AND paye=1 AND YEAR(datef)=".$year." GROUP BY fk_soc, month ORDER BY datef";	
	}
